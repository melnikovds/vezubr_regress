...
def test_transport_edit_lkp(base_fixture, domain):
    ...
    add_ts.input_in_field(add_ts.owner_document_name, value='назв док')
    add_ts.input_in_field(add_ts.owner_document_number, value='№-10235')
    add_ts.input_in_field(add_ts.owner_document_inn, value='665805954074')
    add_ts.click_button(add_ts.owner_document_date)
    time.sleep(1)
    add_ts.click_button(add_ts.owner_document_date_today)
    add_ts.input_in_field(add_ts.owner_document_organization, value='организация ASK')
    ...
    # add_ts.input_in_field(add_ts.calendar_input, "10092045", press_enter=True)
    time.sleep(2)
    add_ts.input_in_field(add_ts.input_date_button_one, "10092045", press_enter=True)
    time.sleep(2)
    ...
    add_ts.click_button(add_ts.side_loading_toggl)
    add_ts.click_button(add_ts.top_loading_toggl)
    add_ts.input_in_field(add_ts.owner_document_number, value='№-78999')
...
@allure.description('ЛКП. Тест создания и редактирования ПП ПВ: '
                    '1) создаем ПП: номер - ПП-timestamp, модель - Полуприцеп, выпуск - 2013г, собственник - Подрядчик,'
...
def test_semitrailer1_edit_lkp(base_fixture, domain):
    ...
    add_ts.dropdown_without_input(add_ts.year_select, "2013")
    add_ts.dropdown_without_input(add_ts.vehicle_categories_select, "Грузовая")
...
def test_semitrailer2_edit_lkp(base_fixture, domain):
    ...
    time.sleep(1)
    add_ts.input_in_field(add_ts.owner_document_name, value='лицензия')
    add_ts.input_in_field(add_ts.owner_document_number, value='89487-90')
    add_ts.input_in_field(add_ts.owner_document_inn, value='550602687266')
    add_ts.click_button(add_ts.owner_document_date)
    time.sleep(1)
    add_ts.click_button(add_ts.owner_document_date_today)
    add_ts.input_in_field(add_ts.owner_document_organization, value='ИП Новичков')
    ...
    time.sleep(1)
    add_ts.backspace_and_input(add_ts.owner_document_name, value='название')
    add_ts.backspace_and_input(add_ts.owner_document_number, value='№-89235')
    add_ts.backspace_and_input(add_ts.owner_document_inn, value='771308767598')
    add_ts.backspace_and_input(add_ts.owner_document_organization, value='ИП Гусев')
...
@allure.description('ЛКП. Тест создания и редактирования Тягача внутр. ПВ: '
                    '1) создаем Тяг: номер - Тяг-timestamp, модель - Тягач, выпуск - 2009г, собственник - Лизинг,'
                    'доп.парам - Нет'
...
def test_tractor_edit_lkp(base_fixture, domain):
    ...
    add_ts.dropdown_without_input(add_ts.year_select, "2009")
    add_ts.dropdown_without_input(add_ts.owner_types_select, "Безвозмездное пользование")
    time.sleep(1)
    add_ts.backspace_and_input(add_ts.owner_document_name, value='паспорт')
    add_ts.backspace_and_input(add_ts.owner_document_number, value='п-443')
    add_ts.backspace_and_input(add_ts.owner_document_inn, value='360800668795')
    add_ts.click_button(add_ts.owner_document_date)
    time.sleep(1)
    add_ts.click_button(add_ts.owner_document_date_today)
    add_ts.backspace_and_input(add_ts.owner_document_organization, value='ИП Дятлов')
    ...
    # Редактирование транспортного средства
    add_ts.click_button(add_ts.edit_button)
    add_ts.dropdown_without_input(add_ts.year_select, "2001")
    add_ts.dropdown_without_input(add_ts.owner_types_select, "Совместная собственность супругов")
    add_ts.dropdown_without_input(add_ts.first_pass_type_select, "МКАД", index=1)
