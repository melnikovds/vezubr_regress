...
def test_ltl_tariff_add_lkp(base_fixture, domain):
    ...
    add_tariff.dropdown_without_input(add_tariff.ltl_type_select, "Мешок")
    ...
    add_tariff.click_and_select_with_arrows(add_tariff.dispatch_region_select, arrow_presses=12)
    time.sleep(1)
    add_tariff.click_outside()
    time.sleep(1)
    # Выбор региона доставки
    add_tariff.click_and_select_with_arrows(add_tariff.delivery_region_select, arrow_presses=8)
    time.sleep(1)
    add_tariff.click_outside()
    time.sleep(1)
    # Ввод минимальной стоимости
    add_tariff.backspace_and_input(add_tariff.min_price_input, base.random_value_float_str(1000, 3000), num=3)
    time.sleep(1)
    # Ввод стоимости за килограмм
    add_tariff.backspace_and_input(add_tariff.kg_price_input, base.random_value_float_str(50, 200), num=3)
    time.sleep(1)
