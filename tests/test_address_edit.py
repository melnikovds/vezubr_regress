...
@allure.description('ЛКЗ. Тест создания адреса: статус - Активный, заполняем поля - Все, редактируем поля - Все')
...
def test_address_edit_lkz(base_fixture, domain):
    ...
    add_address.backspace_and_input(add_address.name_address_input, edit_stamp)
    ...
    add_address.backspace_and_input(add_address.address_input, "")
    add_address.dropdown_with_input(
        add_address.address_input,
        f"г Екатеринбург, пр-кт Ленина, д {base.random_value_float_str(1, 150)}",
        wait_presence=True
    ...
    add_address.backspace_and_input(add_address.owner_inn_input, "")
    add_address.dropdown_with_input(add_address.owner_inn_input, "77", wait_presence=True)
    # Редактирование id адреса партнера
    add_address.backspace_and_input(add_address.external_id_input, address_stamp)
    # Редактирование требований к ТС на адресе
    add_address.backspace_and_input(add_address.max_height_input,
    ...
    add_address.backspace_and_input(add_address.edit_contact_person_input, "Другой Василий")
    add_address.backspace_and_input(add_address.edit_mobile_phone_input,
                                    base.random_value_float_str(1000000000, 9999999999), click_first=True)
    add_address.backspace_and_input(add_address.edit_additional_first_input,
                                    base.random_value_float_str(1, 999999), click_first=True)
    add_address.backspace_and_input(add_address.edit_email_input, f"E{base.get_timestamp()}@mail.ru")
    add_address.backspace_and_input(add_address.edit_work_phone_input,
                                    base.random_value_float_str(1000000000, 9999999999), click_first=True)
    add_address.backspace_and_input(add_address.edit_additional_second_input,
                                    base.random_value_float_str(1, 999999), click_first=True)
