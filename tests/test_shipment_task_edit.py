...
from api_pages.update_entities import UpdateEntities
...
@allure.story("Smoke test")
@allure.feature('Создание и удаление заданий')
@allure.description('ЛКЗ. Тест удаления фактических ГМ из Задания')
@pytest.mark.parametrize('base_fixture', ['lkz'], indirect=True)
def test_shipment_task_with_gm_edit3_lkz(base_fixture, domain):
    # Инициализация базовых объектов через фикстуру
    base, sidebar = base_fixture
    ...
    add_cp = CargoPlaceAdd(base.driver)
    # Добавление полного грузоместа
    cp_stamp_one = add_cp.add_full_cargo_place_lkz()

    # Сброс фильтров
    cp_list.click_button(cp_list.reset_button, wait="lst")
    # Ввод штрихкода грузоместа в поле фильтрации
    cp_list.input_in_field(cp_list.barcode_filter, value=cp_stamp_one, wait="lst")
    # Клик по ссылке первого грузоместа в списке
    cp_list.click_button(cp_list.first_cp_link, wait="form")
    ...
    add_cp = CargoPlaceAdd(base.driver)
    # Добавление полного грузоместа
    cp_stamp_two = add_cp.add_full_cargo_place_lkz()

    # Сброс фильтров
    cp_list.click_button(cp_list.reset_button, wait="lst")
    # Ввод штрихкода грузоместа в поле фильтрации
    cp_list.input_in_field(cp_list.barcode_filter, value=cp_stamp_two, wait="lst")
    # Клик по ссылке первого грузоместа в списке
    cp_list.click_button(cp_list.first_cp_link, wait="form")
    ...
    # Проверяем отображение прикреплённых ГМ в обновлённом Задании
    lst = GmFilters(base.driver)
    lst.dropdown_without_input(lst.required_search_by_date, "За все время")
    time.sleep(2)
    ...
    lst.click_button(lst.first_task_click)
    time.sleep(2)
    # Переходим в таб грузоместа
    tsk = ShipmentTaskAdd(base.driver)
    tsk.click_button(tsk.tab_gm)
    tsk.verify_text_on_page(text=cp_stamp_one)
    tsk.verify_text_on_page(text=cp_stamp_two)

    with allure.step("Удаление 1 ГМ через API"):
        lkz_updater = UpdateEntities(role='lkz')

        updated_task = lkz_updater.update_task(
            task_id=task_id,
            task_number=a,
            cargo_place_ids=[cargo_place_id_two],
            title="updated two title"
        )

    # Проверяем удаление ГМ
    tsk.click_button(tsk.tab_characteristics)
    tsk.reload_page()
    time.sleep(5)
    tsk.click_button(tsk.tab_gm)
    tsk.verify_text_on_page(text=cp_stamp_one, should_exist=False)
    tsk.verify_text_on_page(text=cp_stamp_two, should_exist=True)
